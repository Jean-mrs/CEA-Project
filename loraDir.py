...
#
class myNode:
    def __init__(self, nodeid, bs, period, packetlen, x, y, bsx, bsy):
        self.nodeid = nodeid
        self.period = period
        self.bs = bs
        self.x = 0
...
def lorasim_simulate( nrNodes, data, gtwxy, bs_id, avgSendTime=10, simtime=360000):
    ...
    for i in range(0, nrNodes):
        ...
        # 1000000 = 16 min
        node = myNode(i, bsId, avgSendTime, 20, data[i, 0], data[i, 1], bsx, bsy)
        nodes.append(node)
    ...
    # df = pd.DataFrame()
    # nod_id = pd.Series([n.nodeid for n in nodes])
    # df['nod_id'] = nod_id
    # df['rssi'] = pd.Series([n.packet.rssi for n in nodes])
    # df['sf'] = pd.Series([n.packet.sf for n in nodes])
    # df['bw'] = pd.Series([n.packet.bw for n in nodes])
    # dict_values = df.to_csv('teste.csv', header=True, index=False)

    # with open('rssi3.txt', 'w') as rssifile:
    #     for n in nodes:
    #         rssifile.write(
    #             "\n  node {} | {:3.2f} dBm | sf {} | BW {}".format(n.nodeid, n.packet.rssi, n.packet.sf, n.packet.bw))
    # rssifile.close()
    ...
    # df_nodes['x'] = pd.Series([n.x for n in nodes])
    # df_nodes['y'] = pd.Series([n.y for n in nodes])
    # dict_nodes = df_nodes.to_csv('teste2.csv', header=True, index=False)
    #
