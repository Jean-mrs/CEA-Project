...
def gap_statistics_fuzzy(data, nrefs=3, maxClusters=15):
    ...
    for gap_index, c in enumerate(range(1, maxClusters)):
        ...
        gap = np.mean(refDisps)/nrefs - np.log(np.mean(origDisp))
        ...
        sddk[gap_index] = np.sqrt((np.mean(refDisps)/nrefs) - np.me)
    ...
    return iter_points, iter_points_sk, resultsdf, Fpc, gp, best
